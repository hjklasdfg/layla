...
| **Summary** | Locally-ingested City-of-London open data, fused on-device, exposed as data tools for an accessibility-aware mobility agent: a fused OSM walkable graph + crime/noise/air/accessibility + live TfL. **Route planning is the sibling `layla-routing` skill** — this skill only provides data. |
| **Executable** | `layla_data_skill.py` (pure-Python; JSON I/O) |

## Capabilities (data only)
